...
We're working hard to make *paycore* the most powerful JavaScript library for working with paycoin. Our goal is to have *paycore* be a library that can be used by anyone interested in paycoin, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://labs.bitpay.com/c/paycore (Support Forum)
- https://gitter.im/bitpay/paycore (Development Chat)
...
These are some global design goals in paycore that any change must adhere.
...
When possible, paycore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Paycore methods. For example, calling `JSON.stringify` on an Paycore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'paycore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the paycore object.

DO:
```javascript
var paycore = require('../');
var PublicKey = paycore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add bitpay git@github.com:bitpay/paycore.git
...
```
Finally go to [github.com/bitpay/paycore](https://github.com/bitpay/paycore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of paycore.

If you have any questions feel free to post them to
[github.com/bitpay/paycore/issues](https://github.com/bitpay/paycore/issues).
